...
import { useDQAI } from '@/hooks/useDQAI-v1compat';
import type { ModuleProps } from '@/types/index';
...
export function QualitativeAssessment({ sessionId, data, hooks }: ModuleProps) {
  ...
  const frameGate = (() => { try { return checkFrameGate(data); } catch { return { passes: false, score: 0, reason: 'Frame not ready' }; } })();
  const mechanicalRec = (() => { try { return computeMechanicalRecommendation(data); } catch { return { recommendedStrategy: null, scores: {}, confidence: 'Low' as const, margin: 0, traceable: false, traceability: 'Insufficient data' }; } })();
  ...
  useEffect(() => {
    if (data?.criteria?.length) setCriteria(data.criteria.map((c: any) => {
      const weightMap: Record<number, string> = { 5: 'critical', 4: 'high', 3: 'medium', 2: 'low', 1: 'low' };
      const weight = typeof c.weight === 'number' ? (weightMap[c.weight] ?? 'medium') : (c.weight ?? 'medium');
      return { id: c.id, label: c.label || c.title, type: c.type ?? 'value', weight, description: c.description || '' };
    }));
  ...
  const col = (idx: number | undefined) => sColors[(idx ?? 0) % sColors.length] ?? { fill: '#4F6AF5', soft: '#EEF2FF', line: '#C7D2FE', dark: '#4338CA' };
  ...
  const aiInitAssessment = async () => {
    ...
    const contractRules = (() => { try { return buildContractPrompt('qualitative-assessment', data); } catch { return ''; } })();
...
Score these strategies on each criterion for this decision.\nDecision: "${data?.session?.decisionStatement || ''}"\n\nCriteria:\n${critList}\n\nStrategies:\n${stratList}\n\nReturn JSON: { scores: [{strategyIndex: 1-based, criterionIndex: 1-based, score: 1-5, rationale: string}] }`;
    setBusy(true);
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': import.meta.env.VITE_ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01', 'anthropic-dangerous-direct-browser-access': 'true' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, temperature: 0, system: 'You are a Decision Quality analyst. Respond ONLY with valid JSON.', messages: [{ role: 'user', content: prompt }], _unused_module: 'qualitative-assessment' }) });
      const d = await res.json();
      const _text = (d.content?.find((b: any) => b.type === 'text')?.text || '').replace(/```json/gi, '').replace(/```/g, '').trim();
  ...
  return (
    <div style={{ background: DS.bg, minHeight: '100%' }}>
      {frameGate.score < 30 && (
        <div style={{ margin: '12px 16px', padding: '10px 14px', borderRadius: 10, background: '#FEF3C7', border: '1px solid #FDE68A', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>🔒</span>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#D97706' }}>Complete Problem Frame first (score {frameGate.score}/30)</span>
        </div>
      )}
      {/* Header */}
      <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${DS.border}`, background: DS.surface, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: DS.inkTer, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 }}>Module 05</div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: DS.ink, margin: 0 }}>Qualitative Assessment</h2>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: DS.inkTer }}>{totalScored}/{totalCells} scored</span>
          <div style={{ width: 80, height: 6, borderRadius: 3, background: DS.border, overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 3, background: DS.alternatives.fill, width: `${totalCells ? Math.round((totalScored/totalCells)*100) : 0}%`, transition: 'width 0.3s' }} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={aiInitAssessment} disabled={busy}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: DS.alternatives.fill, color: '#fff', border: 'none', cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.6 : 1 }}>
            <Sparkles size={12} /> {busy ? 'Analyzing…' : 'AI Score All'}
          </button>
          <button onClick={aiAnalyse} disabled={busy}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: DS.surfaceAlt, color: DS.ink, border: `1px solid ${DS.border}`, cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.6 : 1 }}>
            AI Analysis
          </button>
          <button onClick={generateBrief} disabled={busy}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: DS.accent, color: '#fff', border: 'none', cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.6 : 1 }}>
            Decision Brief
          </button>
        </div>
      </div>
      ...
      {mechanicalRec.traceable && mechanicalRec.recommendedStrategy && (
        <div className="rounded-xl p-3 flex items-center gap-3" style={{ background: mechanicalRec.confidence === 'High' ? DS.successSoft : mechanicalRec.confidence === 'Medium' ? DS.warnSoft : DS.bg, border: '1px solid ' + (mechanicalRec.confidence === 'High' ? DS.success : mechanicalRec.confidence === 'Medium' ? DS.warning : DS.borderLight) + '40' }}>
          <div className="flex-1">
            ...
            <div className="text-sm font-bold" style={{ color: DS.ink }}>{mechanicalRec.recommendedStrategy} leads at {mechanicalRec.recommendedStrategy ? (mechanicalRec.scores[mechanicalRec.recommendedStrategy] ?? 0) : 0}/100</div>
                ...
                {criteria.map((c, ci) => (
                  <tr key={c.id} style={{ borderBottom: `1px solid ${DS.borderLight}` }}>
                    <td className="px-3 py-3 w-52" style={{ borderBottom: `1px solid ${DS.borderLight}` }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <Badge style={{ background: `${TYPE_COLORS[c.type] ?? '#64748B'}15`, color: TYPE_COLORS[c.type] ?? '#64748B', border: 'none', fontSize: 8 }}>{String(c.weight ?? 'medium').toUpperCase()}</Badge>
                        <Badge style={{ background: `${TYPE_COLORS[c.type] ?? '#64748B'}15`, color: TYPE_COLORS[c.type] ?? '#64748B', border: 'none', fontSize: 8 }}>{(c.type ?? 'value').toUpperCase()}</Badge>
                      </div>
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-5 gap-1">
                      {[1,2,3,4,5].map(v => {
                        ...
                        return (
                          <button key={v} onClick={() => activeCell && setScore(activeCell.sid, activeCell.cid, v)}
