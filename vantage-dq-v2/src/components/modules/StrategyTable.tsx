    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 2000,
          temperature: 0,
          system: 'You are a Decision Quality expert completing strategy cards. Respond ONLY with valid JSON.',
          messages: [{ role: 'user', content: prompt }],
        }),
      });
      const data2 = await res.json();
      const text = (data2.content?.find((b: any) => b.type === 'text')?.text || '').replace(/```json/gi, '').replace(/```/g, '').trim();
...
Mechanical score: ${mechanicalRec.recommendedStrategy ? mechanicalRec.scores[mechanicalRec.recommendedStrategy] : "N/A"}/100
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 2000,
          temperature: 0,
          system: 'You are a Decision Quality expert. Respond ONLY with valid JSON.',
          messages: [{ role: 'user', content: prompt }],
        }),
      });
...
Return JSON only — no other text:
{
  ...
  "selections": {"decision_id": choiceIndex}
    ...
    try {
      ...
      const rawText = apiData.content?.find((b: any) => b.type === 'text')?.text ?? '{}';
    ...
    <div className="space-y-0">
      ...
      <div className="flex items-start gap-3 mb-4 flex-wrap">
        ...
        <div className="flex gap-1.5 flex-wrap">
          ...
          {activeStratId && <Button size="sm" className="gap-1 text-xs h-7 shrink-0" style={{ background: DS.alternatives.fill, color: '#fff' }} onClick={() => aiFillStrategy(activeStratId!)}><Sparkles size={11} /> AI Fill Strategy</Button>}
        </div>
      </div>
      ...
      {dqResult?.trust && <DQTrustBadge trust={dqResult.trust} />}
