...
export function DQScorecard({ sessionId, data, hooks }: ModuleProps) {
  ...
  const scoreLabel = (s: number) => s >= 90 ? 'Elite' : s >= 70 ? 'Strong' : s >= 45 ? 'Adequate' : s > 0 ? 'Weak' : 'Unscored';

  const aiAutoPopulate = () => {
    const ctx = { decisionStatement: data?.session?.decisionStatement || '', issues: (data?.issues || []).length, strategies: (data?.strategies || []).length, criteria: (data?.criteria || []).length };
    const prompt = `Score this decision on all 6 DQ elements (0-100, use multiples of 10).\nData: ${JSON.stringify(ctx)}\nScoring: 0-19=High-Risk, 20-44=Weak, 45-69=Adequate, 70-89=Strong, 90-100=Elite\nReturn JSON: { frame, alternatives, information, values, reasoning, commitment }`;
    call(prompt, (r) => {
      let result = r;
      if (r?._raw) { try { result = JSON.parse((r._raw || '').match(/\{[\s\S]*\}/)?.[0] || ''); } catch { return; } }
      if (!result || result.error) return;
      const newScores: Record<string, number> = {};
      DQ_ELEMENTS.forEach(el => { if (typeof result[el.key] === 'number') newScores[el.key] = Math.round(result[el.key] / 10) * 10; });
      if (Object.keys(newScores).length) { setScores(p => ({ ...p, ...newScores })); if (sessionId) hooks?.updateSession?.({ id: sessionId, data: { dqScores: { ...scores, ...newScores } } }); }
    });
  };
  ...
  const aiImprovements = () => {
    const weakEls = DQ_ELEMENTS.filter(el => (scores[el.key] || 0) < 70).map(el => `${el.short}: ${scores[el.key] || 0}`).join(', ');
    ...
    <div className="space-y-0">
      ...
      {activeTab === 'scorecard' && (
        <div className="space-y-5">
          ...
          <div className="rounded-2xl p-5 flex items-center gap-6" style={{ background: `linear-gradient(135deg, ${DS.reasoning.soft}, ${DS.canvas})`, border: `1px solid ${DS.reasoning.line}` }}>
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between text-[9px] mt-1" style={{ color: DS.inkDis }}>
                <span>0</span><span>20 Weak</span><span>45 Adequate</span><span>70 Strong</span><span>100 Elite</span>
              </div>
            </div>
            {overall > 0 && (
          ...
          {/* 3 × 2 card grid — 6 element cards */}
          <div className="grid grid-cols-3 gap-4">
            {DQ_ELEMENTS.map(el => {
              const sc = scores[el.key] || 0;
              const color = scoreColor(sc);
              const label = scoreLabel(sc);
              const size = 48; const r = 18; const cx = 24; const cy = 24;
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <svg width={size} height={size} className="shrink-0">
                        ...
                        <text x={cx} y={cy + 4} textAnchor="middle" fontSize={11} fontWeight="800" fill={sc > 0 ? el.fill : DS.inkDis}>{sc}</text>
                      </svg>
                    </div>

                    {/* Description */}
                    <p className="text-[10px] leading-relaxed mb-3 flex-1" style={{ color: DS.inkSub }}>
                      {(el.desc || '').slice(0, 85)}{(el.desc || '').length > 85 ? '…' : ''}
                    </p>
                      ...
                      {SCORE_OPTIONS.map(v => {
                        ...
                        return (
                          <button key={v} onClick={() => setScore(el.key, v)}
                            className="flex-1 py-1.5 rounded text-[8px] font-bold transition-all hover:opacity-80"
                            style={{
                              background: isSelected ? el.fill : DS.bg,
                              color: isSelected ? '#fff' : v >= 70 ? DS.success : v >= 45 ? DS.warning : v > 0 ? DS.danger : DS.inkDis,
                              border: `1px solid ${isSelected ? el.fill : DS.borderLight}`,
          ...
          <div className="flex items-start gap-3 p-3 rounded-xl" style={{ background: `${DS.reasoning.fill}10`, border: `1px solid ${DS.reasoning.fill}25` }}>
            ...
            <div>
              ...
              <p className="text-[11px] leading-relaxed" style={{ color: DS.inkSub }}>
                Would a senior decision advisor sign off on this quality level? A score below 45 on any element means the decision should not be committed to yet. The weakest element determines the ceiling.
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'chain' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-xs" style={{ color: DS.inkSub }}>Radar view + improvement plan for elements below 70.</p>
              ...
              return (
                <div key={el.key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-7 rounded-lg overflow-hidden relative" style={{ background: DS.bg }}>
                    ...
                    <div className="absolute top-0 bottom-0 w-px opacity-40" style={{ left: '45%', background: DS.warning }} />
                    <div className="absolute top-0 bottom-0 w-px opacity-40" style={{ left: '70%', background: DS.success }} />
                  </div>
